...
class InputPolicy(object):
    ...
    def __init__(self, device: "Device", app: "App", allow_to_generate_utg=False):
        ...
        self._event_trace = ""
        self.exploration_tree = ExplorationTree(root_activity="Launcher")  # 初始化活动树
    ...
    def _update_exploration_tree(self):
        """
        更新路径树
        """
        from_activity = self.from_state.foreground_activity if self.from_state else "Unknown"
        to_activity = self.to_state.foreground_activity if self.to_state else "Unknown"
        print(from_activity)
        print(to_activity)
        # 记录事件描述（如按钮文本或动作类型）
        event_desc = self._get_event_description()
        print(event_desc)
        print("尝试更新路径树")
        if from_activity != to_activity:
            self.exploration_tree.add_node(
                parent_activity=from_activity,
                new_activity=to_activity,
    ...
    def _get_event_description(self):
        ...
        返回驱动页面变化的事件类型
        """
        if isinstance(self.last_event, TouchEvent):
            view_text = self.from_state._DeviceState__safe_dict_get(self.last_event.view, 'text', '')
            return f"Touch view with text description : [{view_text[:30]}]" if view_text else "touch view without text description"
        # return "Touch"
        elif isinstance(self.last_event, KeyEvent):
            return "Key"
        elif isinstance(self.last_event, SetTextEvent):
            return f"TextInput : [{self.last_event.text[:40]}]"
        elif isinstance(self.last_event, IntentEvent):
            return "Intent"
        elif isinstance(self.last_event, KillAppEvent):
            return "KillApp"
        elif isinstance(self.last_event, ReInstallAppEvent):
            return "ReInstallApp"
        elif isinstance(self.last_event, KillAndRestartAppEvent):
            return "RestartApp"
        elif isinstance(self.last_event, RotateDeviceToPortraitEvent):
            return "RotateToPortrait"
        elif isinstance(self.last_event, RotateDeviceToLandscapeEvent):
            return "RotateToLandscape"
        elif isinstance(self.last_event, RotateDevice):
            return "RotateDevice"
        else:
            return "Unknown"
...
class RandomPolicy(KeaInputPolicy):
    ...
    def __init__(
            ...
            number_of_events_that_restart_app=114514,
...
class LLMPolicy(RandomPolicy):
    ...
    def __init__(
        ...
        self.from_state = None
        self.task = "You are an expert in application GUI testing. Based on your rich experience in App testing, guide the test tool to improve the coverage of functional scenarios during App testing."
        self.task_description = None
        self.rules_preconditions_text = None
        self.tasks_description_with_preconditions = None
        self.current_tarpit_key = None
        self.ui_tarpit_history = {}
        self.current_tarpit_actions = []
        self.is_in_tarpit = False
    ...
    def start(
        ...
        # 输出任务描述相关的提示词
        print(self.set_llmTaks_with_preconditions(self.kea.tasks_matched))
            ...
            try:
                ...
                if self.event_count == 0:
                    ...
                    if input_manager.sim_calculator.detected_ui_tarpit(input_manager):
                        ...
                        #进入UI陷阱
                        if self.is_in_tarpit == False:
                            self.current_tarpit_key = self._get_tarpit_key()
                        self.is_in_tarpit = True

                        #判断是否进入过该UI陷阱
                        if self.current_tarpit_key not in self.ui_tarpit_history:
                            self.ui_tarpit_history[self.current_tarpit_key] = []
                            self.current_tarpit_actions = []
                            self.logger.info(f"New UI tarpit detected: {self.current_tarpit_key}")
                        else:
                            self.current_tarpit_actions = self.ui_tarpit_history[self.current_tarpit_key]
                            self.logger.info(f"Repeated UI tarpit detected: {self.current_tarpit_key}")

                        # self.current_tarpit_actions.append(self.action_sequence[-1])
                        self.ui_tarpit_history[self.current_tarpit_key] = self.current_tarpit_actions
                        ...
                        if self.is_in_tarpit:
                            self._exit_tarpit()
                        self.logger.info("Using random event")
                        event = self.generate_event()
                ...
                if self.from_state and event:
                    # 更新路径树
                    self._update_exploration_tree()
                    action_desc = self.from_state.get_action_desc(event)
                    self.action_sequence.append(action_desc)
                    self.logger.info(f"Action sequence updated: {action_desc}")
                if self.allow_to_generate_utg:
                    self.update_utg()
    ...
    def _get_action_with_LLM(self, current_state, action_history, activity_history):
        ...
        # avoided_actions = self.ui_tarpit_history.get(self.current_tarpit_key, [])
        # if avoided_actions is not None:
        #     history_prompt = (
        #             f"I have already completed the following steps to leave {activity.split('.')[-1]} page but failed: \n " + ";\n ".join(
        #         avoided_actions) + "Therefore, please do not select components and actions that have already been selected"
        #     )

        # avoided_actions只记录排除的数字序号
        avoided_numbers = self.ui_tarpit_history.get(self.current_tarpit_key, [])
        history_prompt = (
            f"Avoid these action numbers: {', '.join(map(str, avoided_numbers))}"
            if avoided_numbers else ""
        )

        state_prompt, candidate_actions, context_environment_messages, screen_category = current_state.get_described_actions(self.input_manager)
        question = "Which action should I choose next? Just return the action id and nothing else.\nIf no more action is needed, return 0."
        ...
        selected_action = candidate_actions[selected_number]

        # 只保存数字序号到tarpit历史记录
        if self.current_tarpit_key not in self.ui_tarpit_history:
            self.ui_tarpit_history[self.current_tarpit_key] = []
        self.ui_tarpit_history[self.current_tarpit_key].append(selected_number)

        # 重置排除的序号
        if len(self.ui_tarpit_history[self.current_tarpit_key]) >= len(candidate_actions) - 1:
            self.ui_tarpit_history[self.current_tarpit_key] = []
    ...
    def _get_tarpit_key(self):
        #   进入UI陷阱的倒数第二个操作，即进入UI陷阱所在页面的那个事件对应的操作
        #   当前的UI陷阱活动界面
        current_activity = self.from_state.foreground_activity if self.from_state else "None"
        if len(self.action_sequence) >= 2:
            last_second_action = self.action_sequence[-2]
        else:
            last_second_action = "None"
        return last_second_action, current_activity

    def _exit_tarpit(self):
        # 退出UI陷阱
        self.is_in_tarpit = False
        self.current_tarpit_key = None
        self.current_tarpit_actions = []
    ...
    # 生成任务描述和前置条件的提示词
    def set_llmtask_description(self):
        tasks = self.kea.all_llmTasks()
        if self.task_description is None:
            for llmtask in tasks:
                if self.task_description is None:
                    self.task_description = llmtask.description
                else:
                    self.task_description = f"{self.task_description}\n{llmtask.description}"
        return f"Task description:\n{self.task_description}"

    def set_rules_text(self):
        rules = self.kea.all_ruless()
        if self.rules_preconditions_text is None:
            for rule in rules:
                if self.rules_preconditions_text is None:
                    self.rules_preconditions_text = inspect.getsource(rule.function).strip().split("\n", 1)[0]
                else:
                    first_line = inspect.getsource(rule.function).strip().split("\n", 1)[0]
                    self.rules_preconditions_text = f"{self.rules_preconditions_text}\n{first_line}"
        return f"Precondition:\n{self.rules_preconditions_text}"

    def set_llmTaks_with_preconditions(self, tasks_matched: bool):
        tasks = self.kea.all_llmTasks()
        rules = self.kea.all_ruless()
        if self.tasks_description_with_preconditions is None:
            if len(tasks) != len(rules) or not tasks_matched:
                if tasks_matched:
                    self.logger.warning("The number of tasks and rules does not match.")
                self.tasks_description_with_preconditions = f"{self.set_llmtask_description()}\n{self.set_rules_text()}"
            else:
                for llmtask, rule in zip(tasks, rules):
                    if self.tasks_description_with_preconditions is None:
                        first_line = inspect.getsource(rule.function).strip().split("\n", 1)[0]
                        self.tasks_description_with_preconditions = f"Task description and Precondition:\n{llmtask.description}{first_line}"
                    else:
                        first_line = inspect.getsource(rule.function).strip().split("\n", 1)[0]
                        self.tasks_description_with_preconditions = f"{self.tasks_description_with_preconditions}\n{llmtask.description}{first_line}"
        return self.tasks_description_with_preconditions
